---
name: projectb-create-skill
description: "Create or update a requested reusable skill, or explain its structure. For read-only questions, answer directly without starting the creation workflow."
---

**Host applicability:** Resolve the active product and supported configuration before using path or schema examples below. For current Codex, use the installed creation skill or current host documentation and the requested repository location. `.cursor` paths and IDE settings recipes apply only to a verified compatible host; do not create them merely because this migrated reference names them. Preserve existing authorization and unrelated configuration.
...
Before creating a skill, infer the following from the request and repository; ask only for missing information that materially affects the result:
...
2. **Name the requested operation and its boundary**:
   - ✅ Good: "Extract, fill, or merge PDF documents when the user requests one of those operations."
...
```yaml
# PDF Processing
description: Extract, fill, or merge PDF documents when the user requests one of those operations.
...
Use the available clarification tool only for consequential missing information that cannot be inferred. Continue independent preparation while a question is pending.
...
2. Check that the description names a specific requested operation rather than incidental keywords
