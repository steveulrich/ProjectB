...
2. **The codebase signals it.** `pyproject.toml` / `requirements.txt` / `uv.lock` / `.py` files → Python. `package.json` / `tsconfig.json` / `.ts` files → TypeScript. In a polyglot repo, infer the owning subdirectory from the requested integration and ask only if the alternatives materially change scope.
3. **No signal either way.** State a reasonable language default for the requested integration and proceed with reversible preparation. Ask only when the choice materially changes integration scope, runtime, or cost.
...
- **When the SDK is not named:** infer the runtime from the request and repository. State the chosen implementation and continue reversible preparation. Ask only for a consequential ambiguity, and preserve authorization boundaries for provisioning, spending, credentials, or external publication.
