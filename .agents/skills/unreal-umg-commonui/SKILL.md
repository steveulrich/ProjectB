...
description: "Implement or debug Unreal 5.8 UMG/CommonUI widgets, layout, screen lifetime, input routing, focus, local-player ownership, and UI performance."
---
...
8. For affected behavior or the selected acceptance gate, test keyboard/mouse, gamepad, touch if supported, split screen, resolutions, safe zones,
   localization expansion, and accessibility settings.
9. For affected behavior or the selected acceptance gate, profile with Slate Insights/Unreal Insights before applying invalidation or retainers.
