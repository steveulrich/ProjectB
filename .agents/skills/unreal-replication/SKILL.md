...
description: "Implement or diagnose Unreal 5.8 replicated actor/property state, RPCs, authority, ownership, relevancy, or dormancy. Cosmetic UI changes alone do not trigger this skill."
---
...
Consult [`references/authority-ownership.md`](references/authority-ownership.md) when changing authority, owning connections, or RPC routing.
...
8. For affected behavior or the selected acceptance gate, test the supported server topology and remote clients under relevant lag, loss, jitter, reordering, reconnect, and
   relevancy transitions. ProjectB final acceptance retains the four-player listen-server LAN, second-PC, adverse-network, physical-input, and end-to-end gates; add dedicated-server testing only when it is supported or requested.
