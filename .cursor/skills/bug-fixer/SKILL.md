...
| **BF-029** | Combat floating numbers never appear (listen-server / standalone PIE) | Three stacked faults: (1) `ALyraPlayerState::ClientBroadcastMessage` only rebroadcasts when `NetMode==NM_Client`, so listen-server hosts never get number-pop verbs; (2) `LyraHealthSet` sets `Message.Target` to the ASC owner **PlayerState**, and `Cast<AActor>` ran before the pawn lookup so pops spawned at PS origin; (3) Niagara path hardcoded `/Game/Effects/Particles/Impacts/NS_DamageNumbers`, which this project does not ship. | Present locally with `IsLocalPlayerController()` + `UBwayCombatNumberPopComponent::PresentCombatNumber`; use `ClientBroadcastMessage` only for remote clients. Resolve pop location with pawn-first helper (`UBwayCombatReadabilityLibrary::ResolveNumberPopWorldLocation`). Render with pooled `UTextRenderComponent` (no missing Niagara). Skip bots (`GetPlayerController()==nullptr`). Debug: `bway.Combat.DebugNumbers` / `bway.Combat.DebugNumbersRouter`. |
