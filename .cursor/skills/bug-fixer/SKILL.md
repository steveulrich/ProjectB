...
| **BF-031** | UE_MCP_Bridge C2440: cannot convert JSON `KV.Key` to `FString` | UE 5.8 `FJsonObject::Values` keys are `UE::FSharedString`, not `FString`. Copy-init `FString Name = KV.Key` no longer compiles. | Use `FString Name(Key.ToView())` or `FStringView(Key)`. Prefer `TryGetField(TEXT("…"))` over `Values.Find`. Do **not** set `UE_JSONOBJECT_LEGACY_STRING_KEYS` in a plugin — that ODR-mismatches Json.dll. |
